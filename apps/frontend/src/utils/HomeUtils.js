...
const retrieveItemsWithinWeek = async (
  ...
  try {
    setLoading(false);
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_AWS_BACKEND_URL}/items/week`
    );
    setData(response.data.map((item) => ({ ...item, id: item.id })));
...
const retrieveItemsWithinTwoWeeks = async (
  ...
  try {
    setLoading(false);
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_AWS_BACKEND_URL}/items/two_weeks`
    );
    setData(response.data.map((item) => ({ ...item, id: item.id })));
...
const retrieveItemsWithinMonth = async (
  ...
  try {
    setLoading(false);
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_AWS_BACKEND_URL}/items/month`
    );
    setData(response.data.map((item) => ({ ...item, id: item.id })));
...
const retrieveItemsWithinYear = async (
  ...
  try {
    setLoading(false);
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_AWS_BACKEND_URL}/items/year`
    );
    setData(response.data.map((item) => ({ ...item, id: item.id })));
